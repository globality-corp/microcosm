...
if [ "$1" = "test" ]; then
   pip install --no-cache-dir --upgrade --extra-index-url ${EXTRA_INDEX_URL} .\[test\]
   shift
   exec pytest "$@"
elif [ "$1" = "lint" ]; then
   pip install --no-cache-dir --upgrade --extra-index-url ${EXTRA_INDEX_URL} .\[lint\]
   exec flake8 ${NAME}
elif [ "$1" = "typehinting" ]; then
   # Install standard type-linting dependencies
   pip install --no-cache-dir --upgrade --extra-index-url ${EXTRA_INDEX_URL} .\[typehinting\]
   exec mypy ${NAME}
else
   echo "Cannot execute $@"
   exit 3
