...
The secrets file is identified using a `FOO__SECRETS` environment variable
...
def get_config_filename(metadata: Metadata) -> Optional[str]:
    """
    Derive a configuration file name from the FOO__SECRETS
    environment variable.

    """
    envvar = "{}__SECRETS".format(underscore(metadata.name).upper())
    try:
...
def _load_from_file(metadata: Metadata, load_func: Callable[[str], Mapping[str, str]]) -> Dict[Any, Any]:
    ...
    named after the service of the form FOO__SECRETS.
...
def load_from_json_secrets_file(metadata: Metadata) -> Dict[Any, Any]:
    """
